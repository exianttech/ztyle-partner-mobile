...
// utils
import hhmmToDate from '@/utils/hhmmToDate';


const EditAvailableSlots = () => {
  ...
  const [availableSlots, setavailableSlots] = useState([{ start: null, end: null }]);
    ...
    useEffect(() => {
        if (profile?.availableSlots) {
          const converted = profile.availableSlots.map(s => ({
              start: s.start ? hhmmToDate(s.start) : null,
              end: s.end ? hhmmToDate(s.end) : null
          }));
...
export default EditAvailableSlots